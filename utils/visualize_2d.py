...
def draw_multi_label_result(image, preds, gt2D, work_dir, catalog, slice_name, iter_num, dice):
    root_dir = os.path.join(work_dir, slice_name)
    os.makedirs(root_dir, exist_ok=True)
    ...
    array_min = img_np.min()
    array_max = img_np.max()
    normalized_image = (img_np - array_min) / (array_max - array_min + 1e-4)
    scaled_image = (normalized_image * 255).clip(0, 255).astype(np.uint8)
    scaled_image = scaled_image.transpose((1, 2, 0))

    fig, axes = plt.subplots(1, 3, dpi=500, figsize=(12, 4))
    ...
    show_mask(gt_np, axes[1])
    axes[1].set_title('Ground Truth')
    axes[1].axis('off')

    axes[2].imshow(scaled_image)
    show_mask(preds_np, axes[2])
    axes[2].set_title('Prediction')
    axes[2].axis('off')

    fig.subplots_adjust(left=0, right=1, bottom=0, top=1, wspace=0.05, hspace=0)
    base_name = f"{catalog}_iter{iter_num}_dice{dice}"
    plt.savefig(os.path.join(root_dir, f"{base_name}.png"), bbox_inches='tight')
    plt.savefig(os.path.join(root_dir, f"{base_name}.pdf"), bbox_inches='tight')
    plt.close()
