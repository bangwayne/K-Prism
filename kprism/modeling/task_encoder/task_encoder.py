...
class TaskEncoder(nn.Module):
    ...
    def read_memory(self, query_key: torch.Tensor, query_selection: torch.Tensor,
                    ...
                    selector: torch.Tensor, depth: torch.Tensor,
                    frames: torch.Tensor) -> (torch.Tensor, Dict[str, torch.Tensor]):
        """
        query_key       : B * CK * H * W
        ...
        mem_readout, aux_features = self.readout_query(pixel_readout, obj_memory, selector=selector, depth=depth,
                                                       frames=frames)
        # print(aux_features["q_weights"].shape)
        aux_output = {
            'sensory': sensory,
